 ...
 * @author TRC20Token
 * @notice Interfaz mínima para proxy actualizable (upgrade).
 * @dev Permite actualizar la implementación del proxy.
 */
interface IUpgradeableProxy {
    /// @notice Actualiza el proxy a una nueva implementación.
    /// @param newImplementation Dirección del contrato de implementación.
...
contract ProxyAdmin {
    ...
    function upgrade(
        ...
        if (proxyAddress == address(0) || newImplementation == address(0))
            revert ZeroAddress();
        // wake-disable-next-line reentrancy -- Protegido por modifier nonReentrant; proxyAddress es controlado por owner.
        try IUpgradeableProxy(proxyAddress).upgradeTo(newImplementation) {
