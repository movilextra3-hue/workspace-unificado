...
module.exports = async function (deployer, network, accounts) {
  ...
  if (network === 'mainnet') {
    ...
    const addrsPath = path.join(abiDir, 'addresses.json');
    fs.writeFileSync(addrsPath, JSON.stringify({
      tokenAddress: proxy.address,
      implementationAddress: impl.address,
      proxyAdminAddress: admin.address,
      updatedAt: new Date().toISOString()
    }, null, 2));
    console.log('abi/addresses.json creado (compatible con preparar:redespliegue)');
  }
};
