 ...
 * Uso: node scripts/deploy-upgradeable.js mainnet (solo mainnet)
 */
...
async function deploy() {
  ...
  if (networkName !== 'mainnet') {
    console.error('Solo mainnet. Uso: node scripts/deploy-upgradeable.js mainnet');
    process.exit(1);
  }
  const net = MAINNET;
  ...
  const loadArtifact = (name) => {
    ...
    if (!fs.existsSync(p)) throw new Error(`Ejecutar npm run compile primero. Falta ${name}.json`);
    try {
  ...
  const bc = (b) => (b || '').replace(/^0x/, '');
  if (!bc(implArtifact.bytecode) || !bc(proxyArtifact.bytecode) || !bc(adminArtifact.bytecode)) {
    throw new Error('Artifacts sin bytecode. Ejecutar: npm run compile');
  }
  ...
  // Sincronizar abi/addresses.json (fuente para revisar-contratos, set-symbol, check-alignment)
  ...
  console.log('abi/addresses.json creado/actualizado.');
}
