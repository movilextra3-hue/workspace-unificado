#!/usr/bin/env node
/**
 * Genera coincidencia EXACTA TXaXTSUK.
 * Técnicas adicionales: orden de fuentes (AST IDs), runs extendidos, orden invertido.
 * Ref: Solidity #14829 (AST IDs afectan Yul optimizer).
 *
 * Uso: node scripts/generate-exact-match.mjs
 */
...
function compileMultiFile(solc, sources, mainContract, evm, runs, viaIR) {
  const input = {
    language: 'Solidity',
    sources,
    settings: {
  ...
  for (const file of Object.keys(out.contracts || {})) {
    const c = out.contracts[file]?.[mainContract]?.evm?.deployedBytecode?.object;
    if (c) return c.replace(/^0x/, '');
  }
...
// Técnicas: orden init/impl, flattened init-first vs impl-first, AST shift (dummy file), runs ampliados
const SOLC_LIST = [
...
const RUNS_EXTENDED = [0, 1, 50, 99, 100, 101, 149, 150, 151, 198, 199, 200, 201, 202, 250, 299, 300, 301];
...
function buildSources(useOZ, pragmaVer, implFirst, addDummy) {
  ...
  const initP = prepareSource(init);
  const implP = prepareSource(implNoImport);

  const dummy = '// SPDX-License-Identifier: MIT\npragma solidity ' + pragmaVer + ';\nlibrary _DummyAST { }\n';
  const sources = {};
  if (addDummy) sources['_DummyAST.sol'] = { content: dummy };
  if (implFirst) {
    sources['TRC20TokenUpgradeable.sol'] = { content: implP.replace(/import[^;]+;/g, '').replace(/Initializable\.sol["'];\s*\n?/i, '').trim() };
    sources['Initializable.sol'] = { content: initP };
  } else {
    sources['Initializable.sol'] = { content: initP };
    sources['TRC20TokenUpgradeable.sol'] = { content: implP.replace(/import[^;]+;/g, '').replace(/Initializable\.sol["'];\s*\n?/i, '').trim() };
  }
...
function buildMultiFileWithImport(useOZ, pragmaVer, implFirst) {
  const initPath = useOZ && fs.existsSync(path.join(VERIFICATION_DIR, 'Initializable-OZ.sol'))
    ? path.join(VERIFICATION_DIR, 'Initializable-OZ.sol')
    : path.join(ROOT, 'contracts', 'Initializable.sol');
  const implPath = path.join(ROOT, 'contracts', 'TRC20TokenUpgradeable.sol');
  ...
  init = init.replace(/\babstract\s+contract\s+Initializable\b/, 'contract Initializable');
  const initP = prepareSource(init);
  const implP = prepareSource(impl);
  const sources = implFirst
    ? { 'TRC20TokenUpgradeable.sol': { content: implP }, 'Initializable.sol': { content: initP } }
    : { 'Initializable.sol': { content: initP }, 'TRC20TokenUpgradeable.sol': { content: implP } };
  return sources;
}

function buildFlattened(useOZ, pragmaVer, implFirst) {
  const initPath = useOZ && fs.existsSync(path.join(VERIFICATION_DIR, 'Initializable-OZ.sol'))
    ? path.join(VERIFICATION_DIR, 'Initializable-OZ.sol')
    : path.join(ROOT, 'contracts', 'Initializable.sol');
  const implPath = path.join(ROOT, 'contracts', 'TRC20TokenUpgradeable.sol');
...
try {
  console.log('=== Generación coincidencia EXACTA TXaXTSUK ===\n');
  console.log('Técnicas: orden init/impl, flattened impl-first, AST dummy, runs 0-301\n');
  ...
  for (const solcVer of SOLC_LIST) {
    ...
    for (const useOZ of [true, false]) {
      for (const implFirst of [false, true]) {
        for (const addDummy of [false, true]) {
          const multiWithImport = buildMultiFileWithImport(useOZ, pragmaVer, implFirst);
          const flattened = buildFlattened(useOZ, pragmaVer, implFirst);
          if (!multiWithImport && !flattened) continue;

          for (const evm of EVM_LIST) {
            for (const runs of RUNS_EXTENDED) {
              for (const viaIR of [false, true]) {
                for (const useMulti of [true, false]) {
                  const cfg = { solcVer, useOZ, implFirst, addDummy, evm, runs, viaIR, useMulti };
                  try {
                    const solc = await getSolc(solcVer);
                    let compiled;
                    if (useMulti && multiWithImport) {
                      const sources = addDummy
                        ? { '_Dummy.sol': { content: `// SPDX-License-Identifier: MIT\npragma solidity ${pragmaVer};\nlibrary _DummyAST { }\n` }, ...multiWithImport }
                        : multiWithImport;
                      compiled = compileMultiFile(solc, sources, MAIN_CONTRACT, evm || undefined, runs, viaIR);
                    } else if (flattened) {
                    ...
                    checked++;
                    if (checked % 50 === 0) process.stdout.write(`\r  ${checked} (mejor: ${best.prefixMatch} prefix)   `);
  ...
  if (exactMatch) {
    console.log('\n\n✅ COINCIDENCIA EXACTA GENERADA\n');
    console.log('   Source:', exactMatch.useMulti ? 'multi-file' : 'flattened');
    console.log('   Orden:', exactMatch.implFirst ? 'impl-first' : 'init-first');
    console.log('   AST dummy:', exactMatch.addDummy);
...
Orden: ${exactMatch.implFirst ? 'impl-first' : 'init-first'}
AST dummy: ${exactMatch.addDummy}
...
`, 'utf8');
  ...
  if (best.cfg) {
    console.log('   Config:', best.cfg.useMulti ? 'multi' : 'flat', best.cfg.useOZ ? 'OZ' : 'custom',
      'implFirst', best.cfg.implFirst, best.cfg.solcVer?.replace('v', '').split('+')[0],
      best.cfg.evm || 'default', 'runs', best.cfg.runs, 'viaIR', best.cfg.viaIR);
  }
  process.exit(0);
} catch (e) {
  console.error(e);
