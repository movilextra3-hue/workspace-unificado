...
contract('TRC20TokenUpgradeable', (accounts) => {
  ...
  it('batchTransfer transfiere a múltiples direcciones', async () => {
    const bal1Before = BigInt((await token.balanceOf(addr1)).toString());
    const bal2Before = BigInt((await token.balanceOf(addr2)).toString());
    await token.batchTransfer([addr1, addr2], [30, 40]);
    assert.equal((await token.balanceOf(addr1)).toString(), (bal1Before + 30n).toString());
    assert.equal((await token.balanceOf(addr2)).toString(), (bal2Before + 40n).toString());
  });

  it('batchTransfer reverte si longitudes no coinciden', async () => {
    let reverted = false;
    try { await token.batchTransfer([addr1], [10, 20]); } catch (e) { reverted = true; }
    assert.isTrue(reverted);
  });

  it('batchMint crea tokens para múltiples direcciones', async () => {
    const supplyBefore = (await token.totalSupply()).toString();
    await token.batchMint([addr1, addr2], [100, 200]);
    const bal1 = (await token.balanceOf(addr1)).toString();
    const bal2 = (await token.balanceOf(addr2)).toString();
    assert.ok(BigInt(bal1) >= 100n, 'addr1 debe recibir al menos 100');
    assert.ok(BigInt(bal2) >= 200n, 'addr2 debe recibir al menos 200');
    assert.equal((await token.totalSupply()).toString(), (BigInt(supplyBefore) + 300n).toString());
  });

  it('batchMint reverte si longitudes no coinciden', async () => {
    let reverted = false;
    try { await token.batchMint([addr1], [10, 20]); } catch (e) { reverted = true; }
    assert.isTrue(reverted);
  });
  ...
  it('setName y setSymbol actualizan nombre y símbolo', async () => {
    const oldName = await token.name();
    const oldSymbol = await token.symbol();
    await token.setName('NuevoNombre');
    await token.setSymbol('NYS');
    assert.equal(await token.name(), 'NuevoNombre');
    assert.equal(await token.symbol(), 'NYS');
    await token.setName(oldName);
    await token.setSymbol(oldSymbol);
  });
  ...
  it('pauseMinting bloquea mint y batchMint', async () => {
    await token.pauseMinting();
    let reverted = false;
    try { await token.mint(addr1, 10); } catch (e) { reverted = true; }
    assert.isTrue(reverted);
    reverted = false;
    try { await token.batchMint([addr1], [10]); } catch (e) { reverted = true; }
    assert.isTrue(reverted);
    await token.unpauseMinting();
    await token.mint(addr1, 10);
    assert.ok(BigInt((await token.balanceOf(addr1)).toString()) >= 10n);
  });

  it('emergencyPauseAll y emergencyUnpauseAll', async () => {
    await token.emergencyPauseAll();
    let reverted = false;
    try { await token.transfer(addr2, 1); } catch (e) { reverted = true; }
    assert.isTrue(reverted);
    reverted = false;
    try { await token.mint(addr1, 1); } catch (e) { reverted = true; }
    assert.isTrue(reverted);
    await token.emergencyUnpauseAll();
    await token.transfer(addr2, 1);
    await token.mint(addr1, 1);
  });

  it('setMaxTransferAmount limita transferencias', async () => {
    await token.setMaxTransferAmount(50);
    let reverted = false;
    try { await token.transfer(addr2, 100); } catch (e) { reverted = true; }
    assert.isTrue(reverted);
  ...
  it('destroyFrozenFunds quema fondos de dirección congelada', async () => {
    await token.freezeAddress(addr1);
    const bal = (await token.balanceOf(addr1)).toString();
    if (BigInt(bal) > 0n) {
      await token.destroyFrozenFunds(addr1);
      assert.equal((await token.balanceOf(addr1)).toString(), '0');
    }
    await token.unfreezeAddress(addr1);
    await token.mint(addr1, 100);
  });

  it('emergencyFreezeAndBurn congela y quema en una tx', async () => {
    await token.mint(addr2, 200);
    const balBefore = (await token.balanceOf(addr2)).toString();
    assert.ok(BigInt(balBefore) >= 200n, 'addr2 debe tener al menos 200 tras mint');
    await token.emergencyFreezeAndBurn(addr2);
    assert.equal((await token.balanceOf(addr2)).toString(), '0');
    assert.isTrue(await token.frozen(addr2));
    await token.unfreezeAddress(addr2);
  });

  it('getEmergencyStatus devuelve estado correcto', async () => {
    const [p, mp, max, rec] = await token.getEmergencyStatus();
    assert.isFalse(p);
    assert.isFalse(mp);
    assert.equal(max.toString(), '0');
    assert.equal(rec, 'T9yD14Nj9j7xAB4dbGeiX9h8unkKHxuWwb' || rec === '0x0000000000000000000000000000000000000000');
  });

  it('setEmergencyRecoveryAddress y emergencyEvacuate requieren dirección configurada', async () => {
    await token.setEmergencyRecoveryAddress(owner);
    let reverted = false;
    try { await token.emergencyEvacuate(); } catch (e) { reverted = true; }
    assert.isTrue(reverted, 'emergencyEvacuate sin fondos debe revertir NothingToEvacuate');
    await token.setEmergencyRecoveryAddress('T9yD14Nj9j7xAB4dbGeiX9h8unkKHxuWwb');
  });
});
