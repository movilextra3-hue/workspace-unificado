...
Verificación del proyecto frente a la documentación oficial de TRON y estado actual (marzo 2025).
...
| Recurso | URL |
|--------|-----|
| TRC-20 (estándar) | https://developers.tron.network/docs/trc20 |
...
- **TronBox** (`tronbox migrate --network mainnet`) usa solo `fullHost` y `privateKey` en `tronbox.js`; no se configuran cabeceras en el fichero. Para mainnet se recomienda usar el script `npm run deploy:mainnet` (que sí usa la API key si está en `.env`).
...
Las URLs usadas en `post-deploy-perfil.js`, `LO_NECESARIO_TRON.md` y `docs/TRONSCAN_DATOS_PEGAR.md` coinciden con las anteriores.
...
| API Key mainnet | ✅ Soportado | Usar `TRON_PRO_API_KEY` en `.env`; preferir `deploy:mainnet` frente a `tronbox migrate` en mainnet |
