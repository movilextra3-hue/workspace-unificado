...
- **logoPathInRepo**: ruta del logo **desde la raíz del repo**. En monorepo es `blockchain/trc20-token/assets/tether-logo.webp`.

La URL del logo que imprime `npm run post-deploy:perfil` es:

```
https://raw.githubusercontent.com/movilextra3-hue/workspace-unificado/master/blockchain/trc20-token/assets/tether-logo.webp
```
